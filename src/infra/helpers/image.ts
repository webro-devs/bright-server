...
registerFont(
  path.join(__dirname, "./uploads/fonts/Merriweat/Merriweather-Regular.ttf"),
  { family: "MerriweatherRegular", weight: 400 },
);
registerFont(
  path.resolve(__dirname, "./uploads/fonts/Noto/NotoSans-Bold.ttf"),
  { family: "NotoSansBold", weight: 700 },
);
registerFont(path.resolve(__dirname, "./uploads/fonts/Times/Times-Bold.ttf"), {
  family: "TimesBold",
  weight: 700,
});
registerFont(path.resolve(__dirname, "./uploads/fonts/GIloy/Giloy.ttf"), {
  family: "GiloyExtraBold",
  weight: 800,
});
registerFont(path.resolve(__dirname, "./uploads/fonts/Mont/Mont-Bold.ttf"), {
  family: "MontBold",
  weight: 700,
});
...
const ImageWithLogo = async ({ imgPath, imgName }) => {
  try {
    ...
    if (image.width > image.height) {
      ...
      let h = image.height;
      ctx.drawImage(image, x, y, w, h, 0, 0, 1000, 1000);
    } else {
...
const MainImage = async ({ txt = "", ctg = "", imgPath, imgName }) => {
  try {
    ...
    const image = await loadImage(imgPath);
    const logo = await loadImage(path.resolve(__dirname, "./uploads/logo.png"));
    const bottomMask = await loadImage(
    ...
    if (image.width > image.height) {
      ...
      let h = image.height;
      ctx.drawImage(image, x, y, cropImageWidth, h, 0, 0, 1000, 900);
    } else {
      let x = 0;
      let w = image.width;
      let cropImageHeight = (image.width / 10) * 9;
      let y = (image.height - cropImageHeight) / 2;
      ctx.drawImage(image, x, y, w, cropImageHeight, 0, 0, 1000, 900);
    }
